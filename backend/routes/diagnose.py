...
async def diagnose_medical_image(
    ...
    modality: str = Form("auto", description="Requested MedMNIST modality, e.g. skin/chest/retina/pathology"),
    current_user: dict = Depends(get_current_user),
    db = Depends(get_database)
):
    """
    Diagnose disease from uploaded medical image using a selected MedMNIST EfficientNet-B0 model
    ...
    **Response:**
    ...
    try:
        ...
        resolved_modality = result.get('modality', modality)
...
@router.get("/diagnose/model-info")
async def get_diagnosis_model_info():
    """Return loaded/missing MedMNIST modality models and training commands."""
    try:
        info = medical_image_pipeline.diagnosis_service.get_model_info()
        return {
            "success": True,
            "data": info,
        }
    except Exception as e:
        logger.error(f"Model info endpoint error: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
