...
import { 
  ...
  Shield, Baby, Eye, Bone, Brain, Filter, ChevronDown, Check, Sparkles
} from 'lucide-react';
...
export default function Pharmacy() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const data = await pharmacyAPI.getProducts(params);
      if (data.success) {
  ...
  const placeOrder = async () => {
    try {
      ...
      const data = await pharmacyAPI.placeOrder(orderData);
      if (data.order_id || data.order_number) {
        // Upload prescription if available
        if (prescriptionFile && data.order_id) {
          await pharmacyAPI.uploadOrderPrescription(data.order_id, prescriptionFile);
        }
