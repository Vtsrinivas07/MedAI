...
function ActiveDiagnosisBanner() {
  ...
  const suggestedTests = Array.isArray(ad.tests) ? ad.tests.slice(0, 4) : [];
  ...
  return (
    <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 flex flex-wrap items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold dark:text-white text-gray-900">Active Diagnosis: {ad.disease}</p>
        ...
        {suggestedTests.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-2">
            {suggestedTests.map((test, i) => (
              <span key={i} className="px-3 py-1 bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 rounded-lg text-xs font-medium">
                {test}
              </span>
            ))}
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button onClick={() => navigate(routes.prescription_module || '/prescriptions')} className="text-xs text-cyan-300 hover:underline flex-shrink-0">Prescriptions</button>
...
export default function LabTests() {
  ...
  const fetchTests = async () => {
    try {
      ...
      const data = await labTestsAPI.getTests(params);
  ...
  const fetchBookings = async () => {
    try {
      const data = await labTestsAPI.getBookings();
  ...
  const uploadLabReport = async () => {
    ...
    try {
      await labTestsAPI.uploadReport(reportFile, 'Lab Report');
  ...
  const bookTests = async () => {
    ...
    try {
      ...
      const data = await labTestsAPI.createBooking(bookingData);
      if (data.booking_id) {
        // Upload prescription if available
        if (prescriptionFile && data.booking_id) {
          await labTestsAPI.uploadBookingPrescription(data.booking_id, prescriptionFile);
        }
