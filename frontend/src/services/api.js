...
export const diagnosisAPI = {
  completeDiagnosis: async ({ symptoms, imageFile, modality }) => {
    const formData = new FormData();
  ...
  getModelInfo: async () => {
    const response = await api.get('/api/diagnose/model-info');
    return response.data;
  },
};
...
export const pharmacyAPI = {
  ...
  uploadOrderPrescription: async (orderId, file) => {
    const formData = new FormData();
    formData.append('order_id', orderId);
    formData.append('file', file);
    const response = await api.post('/api/orders/upload-prescription', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return response.data;
  },
...
export const labTestsAPI = {
  getTests: async (params = {}) => {
    const response = await api.get('/api/lab-tests/tests', { params });
    return response.data;
  },
  getBookings: async () => {
    const response = await api.get('/api/lab-tests/bookings');
    return response.data;
  },
  uploadReport: async (file, title = 'Lab Report') => {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('title', title);
    const response = await api.post('/api/lab-tests/upload-report', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return response.data;
  },
  ...
  uploadBookingPrescription: async (bookingId, file) => {
    const formData = new FormData();
    formData.append('booking_id', bookingId);
    formData.append('file', file);
    const response = await api.post('/api/lab-tests/upload-prescription', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return response.data;
  },
